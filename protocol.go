...
// Interplantery protocol implementation, crypto and protobuf

type InterplanteryProtocol struct {
    crypto *InterplanteryCrypto
}

func NewInterplanteryProtocol(crypto *InterplanteryCrypto) *InterplanteryProtocol {
    return &InterplanteryProtocol {
        crypto: crypto,
    }
}

func (ipp *InterplanteryProtocol) NewInterplanteryMessage(content string) []byte {
    if ipp.crypto != nil { // If crypto is inited, apply encryption
        return ipp.newEncryptedInterplanteryMessage(content)
    } else {
        return ipp.newPlaintextInterplanteryMessage(content)
    }
}

func (ipp *InterplanteryProtocol) newEncryptedInterplanteryMessage(content string) []byte {
	now := time.Now().Unix()
	...
	interplantery_message := &InterplanteryMessage {
		Type: InterplanteryMessage_ENCRYPTED_TEXT_MESSAGE.Enum(),
...
func (ipp *InterplanteryProtocol) newPlaintextInterplanteryMessage(content string) []byte {
	now := time.Now().Unix()

    // Simple unencrypted text message, just marshal and return
	interplantery_message := &InterplanteryMessage {
		Type: InterplanteryMessage_TEXT_MESSAGE.Enum(),
		TextMessage: &TextMessage {
			Created: &now,
			Text: &content,
		},
...
func (ipp *InterplanteryProtocol) ProcessInterplanteryMessage(msg_bytes []byte) *TextMessage {
	...
	case InterplanteryMessage_TEXT_MESSAGE:
        // Plaintext, just unmarshal
		processed_msg = interplantery_message.TextMessage
	case InterplanteryMessage_ENCRYPTED_TEXT_MESSAGE:
