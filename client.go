...
type InterplanetaryChat struct {
    chat_room string
...
func NewInterplanetaryChat(chat_room string, passphrase string) *InterplanetaryChat {
    var crypto *InterplanetaryCrypto // no crypto by default
    if passphrase != "" { // If passphrase for chat room, init crypto
        // Init crypto
        crypto = NewInterplanetaryCrypto(passphrase, chat_room, NewBHRNG())
    }
    protocol := NewInterplanetaryProtocol(crypto)

    return &InterplanetaryChat {
        chat_room: chat_room,
        protocol: protocol,
        crypto: crypto,
    }
...
func (ip *InterplanetaryChat) Init(ctx context.Context) (err error) {
    ...
    log.Debug("Interplanetary chat object initialized.")

    return nil
}

func (ip* InterplanetaryChat) Discover(ctx context.Context) (err error) {
...
func (ip *InterplanetaryChat) Bootstrap(ctx context.Context) (err error) {
    // Bootstrap DHT 
...
func (ip *InterplanetaryChat) SendMessage(msg string) {
    // Preprocess message here
    log.WithFields(log.Fields{"msg": msg}).Debug("Sending message...")
    msg_bytes := ip.protocol.NewInterplanetaryMessage(msg)
    ip.ps.Publish(ip.pubsubTopic, msg_bytes)
}

func (ip *InterplanetaryChat) outboundMessageLoop(ctx context.Context, outboundMessageChannel chan string) {
...
func (ip *InterplanetaryChat) inboundMessageLoop(ctx context.Context, inboundMessageChannel chan string) {
    ...
    for {
        ...
        default:
            ...
            // Unmarshal message
            ip_msg := ip.protocol.ProcessInterplanetaryMessage(msg.Data)
            if ip_msg == nil {
                log.WithFields(log.Fields {
                    "msg_bytes": msg.Data,
...
func (ip *InterplanetaryChat) StartReceiving(ctx context.Context, inboundMessageChannel chan string) {
    // Start message loop - only call this once
    go ip.inboundMessageLoop(ctx, inboundMessageChannel)
}

func (ip *InterplanetaryChat) StartSending(ctx context.Context, outboundMessageChannel chan string) {
    // Start message loop - only call this once
...
func main() {
    ...
    ip_chat := NewInterplanetaryChat(*chat_room, *room_passphrase)
