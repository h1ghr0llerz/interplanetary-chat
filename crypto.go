...
type InterplanteryCrypto struct {
	master_key []byte
...
func NewInterplanteryCrypto(passphrase string, saltphrase string, randReader io.Reader) *InterplanteryCrypto {
	salt_hash := sha1.Sum([]byte(saltphrase))
	derived_key := pbkdf2.Key([]byte(passphrase), salt_hash[:], 4096, BLOCK_SIZE, sha1.New) // 32 byte key

	ic := &InterplanteryCrypto {
...
func (ic *InterplanteryCrypto) DecryptMessage(message_key_ct, iv, data []byte, unpadded_length int64) []byte {
...
func (ic *InterplanteryCrypto) EncryptMessage(data []byte) (ciphertext, iv, message_key_ct []byte) {
